...
MINI_MATRIX_C = np.zeros(
    (16, 4), dtype=MATRIX_DTYPE
)  # Initialize MINI_MATRIX_C with zeros
...
def split_matrix(matrix, axis, rank, size):
    if axis == "r":
        ...
        return matrix[
            ...
            :,
        ].copy()  # copy so the other matrix can be freed
    elif axis == "c":
...
def get_step_indices(start, k, partitions, width):
    assert k % (partitions * width) == 0, "The number of partitions and their width must split k"
    selected_indices = []
