...
def main():
    ...
    num_rows = 3
    num_cols = 3
    assert num_rows * num_cols == size, f"The number of processors must be turned into a {num_rows}x{num_cols} grid."

    row_groups = []
    col_groups = []

    for i in range(num_rows):
        row_groups.append(comm.Create(comm.group.Incl(range(i * num_cols, (i + 1) * num_cols))))
    ...
    strip_width = 1
    assert k % strip_width == 0, "The strip dimension must divide along the K dimension."

    col_block = rank % num_cols
    row_block = rank // num_cols

    A_I = MATRIX_A[row_block * num_cols : (row_block + 1) * num_cols, col_block * num_rows : (col_block + 1) * num_rows].copy()
    B_I = MATRIX_A[row_block * num_cols : (row_block + 1) * num_cols, col_block * num_rows : (col_block + 1) * num_rows].copy()
    C_I = MATRIX_A[row_block * num_cols : (row_block + 1) * num_cols, col_block * num_rows : (col_block + 1) * num_rows].copy()
    ...
    for i in range(k // strip_width):
        ...
        Atmp = None
        if strip_location >= row_block * num_cols and strip_location < (row_block + 1) * num_cols:
            row_block[row_block].bcast()

        Btmp = None
        # check for B_I
        if strip_location >= col_block * num_rows and strip_location < (col_block + 1) * num_rows:
            pass
